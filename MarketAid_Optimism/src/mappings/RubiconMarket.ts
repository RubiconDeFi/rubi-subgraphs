...
import { LogMake, LogTake, LogKill, OfferDeleted, FeeTake, emitOffer, emitTake, emitCancel, emitFee, emitDelete } from "../../generated/RubiconMarket/RubiconMarket"
...
export function handleTake(event: emitTake): void {
    ...
    if (takerAid) {
        ...
        if (assetSoldHistory) {
            ...
            assetSoldHistory.token = buyGem.id
        ...
        if (assetReceivedHistory) {
            ...
            assetReceivedHistory.token = payGem.id
    ...
    if (makerAid) {
        ...
        if (assetSoldHistory) {
            ...
            assetSoldHistory.aid_token = assetSold.id
            assetSoldHistory.token = payGem.id
            assetSoldHistory.transaction = transaction.id
            assetSoldHistory.balance = assetSold.balance
            assetSoldHistory.balance_change = NEG_ONE_BI.times(amountSold)
            assetSoldHistory.transaction = transaction.id
        ...
        if (assetReceivedHistory) {
            ...
            assetReceivedHistory.aid_token = assetReceived.id
            assetReceivedHistory.token = buyGem.id
            assetReceivedHistory.transaction = transaction.id
            assetReceivedHistory.balance = assetReceived.balance
            assetReceivedHistory.balance_change = amountReceived
            assetReceivedHistory.transaction = transaction.id
...
export function handleFee(event: emitFee): void {
    ...
    if (!feeToken) {
        ...
        if (!feeTokenHistory) {
            ...
            feeTokenHistory.token = feeAsset.id
