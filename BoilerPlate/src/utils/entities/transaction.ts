...
import { toBigDecimal } from '../entities/token'
...
export function fetchTransaction(event: ethereum.Event): Transaction {
    ...
    if (transaction == null) {
        ...
        let ethPrice = BigDecimal.fromString('0')
        let ethTokenPrice = getUsdPricePerToken(WETH)

        // check if reverted or not 
        if (!ethTokenPrice.reverted) {
            ethPrice = ethTokenPrice.usdPrice.div(
                ethTokenPrice.decimalsBaseTen
            );
        } else {
            ethPrice = ethTokenPrice.usdPrice
        }
        ...
        let l2ExecutionFeeUSD = l2ExecutionFee.times(ethPrice)
        ...
        let l1FeeUSD = l1Fee.times(ethPrice)
        ...
        let totalGasFeeUSD = totalGasFee.times(ethPrice)
        ...
        transaction.eth_price = ethPrice
        transaction.data = event.transaction.input
